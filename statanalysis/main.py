    ...
    for i in range(p):
        for j in range(len(quadrate)):
            if y_quadrate[i] == quadrate[j]:
                points.append([i, j])
                # print('x =', i, '=> ', 'y^2 =', (i ** 3 + a * i + b),
                # 'mod', p, # '=', (i ** 3 + a * i + b) % p, '  y =', j)
                # комментарий должен начинаться с '# ', а не '#' и длина строки превышает рекомендуемую
...
def point_orders(a, b, p):   # перед функцией стоит три пустых строчки вместо рекомендуемых двух
    ...
    for i in points:
        ...
        for l in range(2, curve_order + 1):
            T = i
            T_ = T   # между оператором и названием переменной отсетствует пробел
            l_ = bin(l)[3:]
